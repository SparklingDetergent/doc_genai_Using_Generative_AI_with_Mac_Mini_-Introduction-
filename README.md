...
- Mac Apple siliconにより構成されたMac Miniを用いてローカルLLMによる生成AIチャット機能を導入し、機密情報を含めて安全に利用できる環境を構築することで、業務効率化を実現することを目指します。
<br/><br/>
- 「◇」マークについては、この目的に沿って着目するべきキーワードに記しています。
  - 「◇」マークで検索するとマーカー色で表示されるので見やすいです。
<br/><br/>
- 当資料では主なキーワードのみ記しているため、詳細をご覧になりたい場合は、各リンク資料をご参照ください。
...
- いったん回答が終わったら、「続けて」と入力し実行してみましょう。
  - 回答が出尽くしたと感じたら、「これまでの回答を総合して最高の結果を出してください」と入力して実行してみましょう。
<br/><br/>
