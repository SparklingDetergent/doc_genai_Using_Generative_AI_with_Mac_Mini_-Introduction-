...
**プロンプト:** Powershellでファイル又はフォルダの削除処理を実装してください。レベル１から順に完成度を高めるものとします。
...
```
* 指定されたファイル"test.txt"を削除します。

**レベル2：特定の拡張子のファイルを一括削除**

```powershell
Get-ChildItem "C:\temp" -Filter "*.txt" | Remove-Item
```
* "C:\temp"フォルダ内のすべての.txtファイルを削除します。
